...
function ResultsContent() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md">
          ...
          <button onClick={() => router.push('/dashboard')} className="w-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700">
            Back to Dashboard
          </button>
        </div>
      </div>
  ...
  const getScoreBg = (score: number) => score >= 90 ? 'bg-green-50 border-green-200' : score >= 70 ? 'bg-yellow-50 border-yellow-200' : 'bg-red-50 border-red-200';
  const getSeverityColor = (s: string) => s === 'high' ? 'border-red-500 bg-red-50' : s === 'medium' ? 'border-orange-500 bg-orange-50' : 'border-yellow-500 bg-yellow-50';
  const getSeverityBadge = (s: string) => s === 'high' ? 'bg-red-100 text-red-800' : s === 'medium' ? 'bg-orange-100 text-orange-800' : 'bg-yellow-100 text-yellow-800';
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      <nav className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <button onClick={() => router.push('/dashboard')} className="text-gray-600 hover:text-gray-900">← Back to Dashboard</button>
        </div>
      </nav>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-5xl mx-auto">
          ...
          <div className={`rounded-lg border-2 p-8 mb-8 ${getScoreBg(results.score)}`}>
            <div className="text-center">
              <p className="text-gray-700 text-lg mb-2">Overall Quality Score</p>
              <p className={`text-6xl font-bold ${getScoreColor(results.score)}`}>{results.score}%</p>
              <p className="text-gray-600 mt-4">
                {results.score >= 90 ? 'Excellent data quality!' : results.score >= 70 ? 'Good data quality with minor issues' : 'Data quality needs improvement'}
              </p>
              <p className="text-sm text-gray-500 mt-2">{results.total_issues} total issues detected</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {results.details && Object.entries(results.details).map(([key, detail]: [string, any]) => (
                <div key={key} className={`border-l-4 p-4 rounded ${getSeverityColor(detail.severity)}`}>
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-semibold text-gray-800 capitalize">{key.replace(/_/g, ' ')}</h4>
                    <span className={`text-xs px-2 py-1 rounded ${getSeverityBadge(detail.severity)}`}>{detail.severity}</span>
                  </div>
              ...
              {results.details?.missing_values?.count > 0 && (
                <div className="flex items-start p-3 bg-red-50 rounded">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{results.details.missing_values.count} missing values found. Consider imputation or data collection improvements.</p>
                  </div>
                </div>
              )}
              {results.details?.type_errors?.count > 0 && (
                <div className="flex items-start p-3 bg-orange-50 rounded">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{results.details.type_errors.count} type mismatches detected. Ensure data types match expected formats.</p>
                  </div>
                </div>
              )}
              {results.score >= 90 && (
                <div className="flex items-start p-3 bg-green-50 rounded">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Your data quality is excellent and ready for analysis and reporting.</p>
                  </div>
                </div>
              )}
            </div>
...
export default function Results() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center"><div className="animate-spin rounded-full h-16 w-16 border-b-2 border-indigo-600"></div></div>}>
      <ResultsContent />
    </Suspense>
  );
}